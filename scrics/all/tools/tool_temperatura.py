...
def consultar_clima(ciudad, dias=3):
    script_path = os.path.join(os.path.dirname(__file__), '..', 'pronostico_temperatura.py')
...
if __name__ == "__main__":
    ...
    print("\n=== VALIDACION MANUAL ===")
    test_preguntas = [
        "Cuantos habitantes tiene Barcelona?",  # NO debe usar tool
        "Que temperatura hace en Barcelona?",    # SI debe usar tool
    ]
    
    for p in test_preguntas:
        es_clima = es_pregunta_climatica(p)
        print(f"'{p}' -> Es climatica: {es_clima}")
    
    # Pruebas que SI deberian usar la tool (clima)
    preguntas_clima = [
        "Que temperatura hace en Barcelona?",
        "Va a llover manana en Madrid?",
        "Pronostico de 5 dias para Valencia",
        "Que tiempo va a hacer esta semana en Sevilla?",
        "Temperatura para los proximos 2 dias en Bilbao"
    ]
    
    # Pruebas que NO deberian usar la tool (otros temas)
    preguntas_otros = [
        "Cuantos habitantes tiene Barcelona?",
        "Cual es la poblacion de Madrid?",
        "Como estas?",
        "Cual es la capital de Espana?",
        "Que hora es?",
        "Cuanta gente vive en Valencia?"
    ]
    
    print("\n=== PREGUNTAS DE CLIMA (deben usar tool) ===")
    for pregunta in preguntas_clima:
        print(f"\nPregunta: {pregunta}")
        respuesta = chat_simple(pregunta)
        print(f"Respuesta: {respuesta[:100]}...")
    
    print("\n=== PREGUNTAS NO-CLIMA (no deben usar tool) ===")
    for pregunta in preguntas_otros:
        print(f"\nPregunta: {pregunta}")
        respuesta = chat_simple(pregunta)
        print(f"Respuesta: {respuesta[:100]}...")